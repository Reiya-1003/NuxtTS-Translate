  ...
  firebase.auth().onAuthStateChanged(function(user) {
    ...
    methods:{
        deleteItem: function():void{   //削除処理
            ...
            const db = firebase.firestore()
       　　　let dbLists = db.collection("users").doc(`${self.username}`).collection("list") //コレクション指定
            let result =window.confirm("Delete？？？")　　//okを押すとtrueがはいる　キャンセルを押すと falseがはいる
　　　　　 if( result ) {
            //console.log('OKがクリックされました');
        　　　dbLists.doc(`${self.source.id}`).delete()　//doc("FBの自動設定データidの指定")
             self.byouga = false　//オブジェクトの変更があった際にNuxt側が認識するためにtrue→falseにかえる
