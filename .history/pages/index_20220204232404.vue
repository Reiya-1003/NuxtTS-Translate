...
　　<div>
    ...
    　　　　 <div>
     　　　　　　<form class="areaen" @submit.prevent="Jachange">
       　　　　　　　　<textarea v-model="list.english" placeholder="English" class="areaen__texten"></textarea>
     　　　　　　</form>
     　　　　 </div>
...
　　　　
  Enchange: function(this:any){
    ...
    console.log(kara)
    axios.get(`${process.env.GASAPI}${kara}&source=ja&target=en`)
   ...
   Jachange: function(this:any){
    ...
    axios.get(`${process.env.GASAPI}${this.list.english}&source=en&target=ja`)
         .then((res:any)=>{ //型定義とばすためのany
           console.log(res.data)
