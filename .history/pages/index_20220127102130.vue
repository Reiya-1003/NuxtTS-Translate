...
　　　<div>
     <div>
     <form>英語訳
       <input type="text" v-model="japan">
       <button type="button" @click="Enchange">翻訳</button>
     </form>
     <div>翻訳結果：{{items.text}}</div>
...
const kara:any = ""
console.log(kara)
const name = encodeURI(``)//日本語はエンコードURIしないとurlで使えない

interface TransInfo{
  japan: number | string,
  english: number | string,
  items: number | string | object
...
// 　　　　
  Enchange: function(this:any){
   ...
   Jachange: function(this:any){
    let kara:any = encodeURI(this.message)
    console.log(kara)
    axios.get(`https://script.google.com/macros/s/AKfycbxSjnREpITlKTl9lo_uV2yqnQbzh4SH7jbAl8fN6YIHHiGWsTDl/exec?text=${this.english}&source=ja&target=en`)
         .then((res:any)=>{
           console.log(res.data)
